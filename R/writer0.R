...
wrShFunc <- function() {
  glue::glue(
    ...
    '    }} \n',
    '    # The "group" and "sub" columns are now in "val1" \n',
    '    # and the column whose name starts with "sub", so rename them \n',
    '    colnames(ggData)[colnames(ggData) == "val1"] <- "group" \n',
    '    colnames(ggData)[startsWith(colnames(ggData), "sub")][1] <- "sub" \n',
    '    by_columns <- c("group", "sub") \n',
    '    ggData$express = FALSE \n',
    '    ggData[val2 > inpcut]$express = TRUE \n',
    '    ggData1 = ggData[express == TRUE, .(nExpress = .N, avgExpress = mean(val2)), by = by_columns] \n',
    '    ggData = ggData[, .(nCells = .N), by = by_columns] \n',
    '    ggData = ggData1[ggData, on = by_columns] \n',
    '    ggData[is.na(nExpress)]$nExpress = 0 \n',
    '    ggData$pctExpress = 100*ggData$nExpress/ggData$nCells \n',
    '    ggData = ggData[, c("group", "sub", "nCells", "nExpress", "pctExpress", "avgExpress"), with = FALSE] \n',
    ...
    'readGeneLookupTable <- function(props) {{ \n',
    ...
    'toLinkouts <- function(gene, props, gene_lookup_table) {{ \n',
    ...
    '  }} \n',
    '  linkouts_url <- paste0("https://services.lis.ncgr.org/gene_linkouts?genes=", props$gene_prefix, gene) \n',
    ...
    '    if (gene %in% names(gene_lookup_table)) {{ \n',
    '      # substitute full-yuck gene symbol if available \n', 
    '      linkouts$href <- gsub(gene, gene_lookup_table[[gene]], linkouts$href) \n',
    '    }} \n',
    '    if (!is.null(props$gene_prefix)) {{ \n',
    '      # remove gene prefix for display \n',
    '      linkouts$text <- gsub(props$gene_prefix, "", linkouts$text) \n',
    '    }} \n',
    '    linkouts_html <- paste( \n',
    '      "<dl>", \n',
