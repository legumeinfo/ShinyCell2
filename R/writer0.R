...
wrShFunc <- function() {
  glue::glue(
    ...
    '    # and the column whose name starts with "sub" \n',
    '    ggColumns <- colnames(ggData) \n',
    '    sub_column <- ggColumns[startsWith(ggColumns, "sub")][1] \n',
    '    ggData$express = FALSE \n',
    '    ggData[val2 > inpcut]$express = TRUE \n',
    '    ggData1 = ggData[express == TRUE, .(nExpress = .N), by = c("val1", sub_column)] \n',
    '    ggData2 = ggData[, "avgExpress" := lapply(.SD, mean), .SDcols = c("val2"), by = c("val1", sub_column)] \n',
    '    ggData = ggData[, .(nCells = .N), by = c("val1", sub_column)] \n',
    '    ggData = ggData1[ggData, on = c("val1", sub_column)] \n',
    '    ggData = ggData2[ggData, on = c("val1", sub_column)] \n',
    '    ggData = ggData[, c("val1", sub_column, "nCells", "nExpress", "avgExpress"), with = FALSE] \n',
    '    ggData = unique(ggData) \n',
    '    ggData[is.na(nExpress)]$nExpress = 0 \n',
    '    ggData$avgExpress = round(ggData$avgExpress, 3) \n',
    '    ggData$pctExpress = round(100 * ggData$nExpress / ggData$nCells, 2)\n',
    '    # Rename the "group" and "sub" columns \n',
    '    colnames(ggData)[1:2] = c("group", "sub") \n',
    '    colnames(ggData)[4] = paste0(colnames(ggData)[4], "_", inp2) \n',
